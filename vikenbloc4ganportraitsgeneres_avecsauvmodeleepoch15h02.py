# -*- coding: utf-8 -*-
"""vikenbloc4ganportraitsgeneres_avecsauvmodeleepoch15h02.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1aut3kpxFcTQlY_ed5YoGN9heXWPqnQBw
"""
...
save_path = "/content/drive/MyDrive/Bloc4Viken/dcgan_checkpoint_12h27.pth"
images_save_path = "/content/drive/MyDrive/Bloc4Viken/generated_images_12h27"  # Chemin pour sauvegarder les images
...
# Création du répertoire pour sauvegarder les images si nécessaire
if not os.path.exists(images_save_path):
    os.makedirs(images_save_path)

# Chargement du modèle si disponible
epoch_start = 0  # Initialiser l'époque de départ

if os.path.exists(save_path):
    checkpoint = torch.load(save_path, map_location=device)
    epoch_start = checkpoint['epoch'] + 1  # Reprendre à partir de l'époque suivante
    generator.load_state_dict(checkpoint['generator'])
    discriminator.load_state_dict(checkpoint['discriminator'])
    optimizer_g.load_state_dict(checkpoint['optimizer_g'])
    optimizer_d.load_state_dict(checkpoint['optimizer_d'])
    print(f"Modèle chargé avec succès. Reprise de l'entraînement à l'époque {epoch_start}.")

# Fonction pour afficher et sauvegarder les images générées pendant l'entraînement
def show_and_save_generated_images(epoch, i, fake_images):
    ...
    img_filename = f"epoch_{epoch}_batch_{i}.png"
    img_path = os.path.join(images_save_path, img_filename)
    vutils.save_image(fake_images, img_path, normalize=True)

# Entraînement
for epoch in range(epoch_start, num_epochs):  # Commencer à partir de l'époque sauvegardée
    for i, (real_images, _) in enumerate(dataloader):
        ...
        if i % 50 == 0:  # Afficher les résultats tous les 50 batches
            ...
            # Afficher et sauvegarder les images générées
            with torch.no_grad():
                fake_images = generator(torch.randn(16, nz, 1, 1, device=device))
                show_and_save_generated_images(epoch, i, fake_images)

    # Sauvegarder le modèle à la fin de chaque époque
    torch.save({
        'epoch': epoch,  # Sauvegarder l'époque
