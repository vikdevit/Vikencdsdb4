...
Original file is located at
    https://colab.research.google.com/drive/1yi2Nf1TJGney3ecKMVLAQyJblunUnJKQ
"""
...
-Erreur moyenne absolue élevée. L'écart entre loss et val_loss est significatif (mae_train = 7.9895 ans et mae_validation = 8.5913 ans) ce qui suggère un sur-apprentissage du modèle de régression qui est meilleur sur les données d'entraînement mais perd en précision sur la validaiton et la présentation d'images que le modèle n'a pas vues. La loss d'entraînement est plus basse que la loss de validation. Le modèle s’adapte trop aux images UTKFace mais ne généralise pas bien.
...
-Ajout d'une couche de convolution avec 256 filtres
...
# avant dernier : layers.Dropout(0.1),

early_stopping = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)

model5_regression.compile(optimizer='adam', loss='mse', metrics=['mae'])
#model3_regression.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.00e'])
...
# ci-dessous version modèle avec régularisation L2 et Drop out
"""model_regression = Sequential([
    layers.InputLayer(input_shape=(img_height, img_width, 3)),
    data_augmentation,

    layers.Conv2D(32, 3, activation='relu', kernel_regularizer=l2(0.001)),
    layers.BatchNormalization(),
    layers.MaxPooling2D(),
    ...
    layers.Conv2D(256, 3, activation='relu', kernel_regularizer=l2(0.001)),
    layers.BatchNormalization(),
    layers.MaxPooling2D(),
